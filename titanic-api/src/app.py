...
from .config import app_config, get_database_url
...
def create_app(env_name: str) -> Flask:
    ...
    # Override config values that need to be evaluated at runtime (after env vars are loaded)
    ...
    logger.info(f"SQLALCHEMY_DATABASE_URI from config: {app.config.get(db_uri_key)}")

    db.init_app(app)

    # Initialize observability (with app context available)
    if OBSERVABILITY_AVAILABLE:
        try:
            init_observability(app, db_instance=db, log_level=os.getenv("LOG_LEVEL", "INFO"))
        except Exception as e:
            logger.warning(f"Failed to initialize observability: {e}. Continuing without observability.")
    ...
    def health():
        ...
        Returns immediately to avoid Kubernetes probe timeouts
        
        Returns:
            Health status
        """
        # Simple health check - app is running and responding
        # Database connectivity is verified on actual API requests
        return {"status": "healthy", "service": "titanic-api"}, 200
