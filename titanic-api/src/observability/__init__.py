...
def init_observability(app, db_instance=None, log_level: str = "INFO") -> None:
    ...
    try:
        ...
        # Register metrics endpoint FIRST (before tracing, which might fail)
        app.add_url_rule("/metrics", "metrics", metrics_endpoint)
        logger.info("Metrics endpoint registered at /metrics")
        
        # Setup tracing (may fail if db_instance not available, but that's OK)
        try:
            setup_tracing(app, db_instance)
        except Exception as e:
            logger.warning(f"Tracing setup failed (non-critical): {e}. Continuing without tracing.")
